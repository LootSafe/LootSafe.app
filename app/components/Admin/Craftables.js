...
export default class Craftables extends Component {
  constructor(props) {
    super(props);
    this.state = {
      items: [],
        ...
        fetch(`${apiAddr}/item/ledger`)
          ...
          .then(ledger => {
            ...
            craftables.map(craftable => {
              ...
              if (item[0]) {
                items.push(item[0]);
                fetch(`${apiAddr}/recipe/get/${item[0].address}`)
                  .then(res => res.json())
                  .then(recipe => {
                    this.setState({
  ...
  render() {
    return (
      <div className="wtf">
        ...
        <h2 style={{ float: 'left' }}>Crafting Recipes</h2>
